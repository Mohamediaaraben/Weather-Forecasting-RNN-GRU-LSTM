""" THURSDAY JAN 25 19:09:08 2025; By MOHAMED IAARABEN """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="use embedded date in feature",
    )
    parser.add_argument(
        "-p",
        "--predict_x_hours",
        ...
        default=24,
        type=int,
        help="value to predict for next x hours",
    )
    oj_args = parser.parse_args(sys.argv[1:])

    print(
        "INFO: model will predict for {} hours' temperature".format(
            oj_args.predict_x_hours
        )
    )
    in_predict_value = oj_args.predict_x_hours * 6

    oj_kmodel = LSTMRnn(0.8, 0.1, 0.1, 2016, in_predict_value, 3, 128)
    oj_kmodel.read_data(
        oj_args.path,
        use_date=oj_args.use_date,
        use_embedded_date=oj_args.use_encoded_date,
    )
    ...
    if (not oj_args.use_date and not oj_args.use_encoded_date) or (
        ...
        oj_kmodel.keras_lstm()
        oj_kmodel.fit_model_generator(out_ftag="LSTM")
    elif not oj_args.use_date and oj_args.use_encoded_date:
        oj_kmodel.merge_model()
        oj_kmodel.keras_lstm_merge_model()
        oj_kmodel.fit_model_generator_merge_model(out_ftag="LSTM")
    else:
